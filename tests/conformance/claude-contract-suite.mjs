/**
 * Claude-style API contract — every case performs real work (bytes on disk,
 * bytes on the wire, subprocess I/O, async continuation, worker messages,
 * compression roundtrip, stream plumbing, etc.).
 *
 * Run via:       node tests/conformance/test-claude-api-contract.mjs (runner)
 * Run (source):  (same — this file is imported by the runner)
 ...
 * Multi-env: node scripts/run-with-env.mjs A=1 B=2 tests/conformance/test-claude-api-contract.mjs
 */
...
export async function runClaudeApiContract() {
...
const { test, testAsync, assert, assertEq, assertDeepEq, assertThrows, finish } =
  createHarness('Claude-style API contract (behavioral)');

printConformanceTarget('claude-contract');
...
const tmp = (name) => `/tmp/claude_contract_${Date.now()}_${name}`;
...
test('fs.writeFileSync wx / r+ / append (claude mailbox patterns)', () => {
...
await testAsync('https.get live TLS fetch (httpbin GET + parse JSON body)', async () => {
  if (offline) throw new HarnessSkip('CLAUDE_CONTRACT_OFFLINE=1');
...
await testAsync('https.request live POST JSON (httpbin POST)', async () => {
  if (offline) throw new HarnessSkip('CLAUDE_CONTRACT_OFFLINE=1');
  const payload = JSON.stringify({ claudeContract: true, t: Date.now() });
  const body = await new Promise((resolve, reject) => {
  ...
  const parsed = JSON.parse(body);
  assertDeepEq(parsed.json?.claudeContract, true);
});
...
test('zlib gzipSync + gunzipSync roundtrip', () => {
  const input = Buffer.from('claude contract payload'.repeat(20), 'utf8');
...
await testAsync('child_process.exec async callback', async () => {
  ...
  fs.mkdirSync('/tmp/claude_exec_cb', { recursive: true });
  fs.writeFileSync('/tmp/claude_exec_cb/in.txt', 'exec-cb');
  await new Promise((resolve, reject) => {
    child_process.exec('cat /tmp/claude_exec_cb/in.txt', { encoding: 'utf8' }, (err, stdout) => {
      if (err) reject(err);
      else {
...
if (mode !== 'node') {
  test('child_process.execSync reads MEMFS (real shell shim)', () => {
    fs.writeFileSync('/tmp/claude_shim.txt', 'shim-ok');
    const out = child_process.execSync('cat /tmp/claude_shim.txt', { encoding: 'utf8' });
...
if (mode === 'node') {
  await testAsync('worker_threads Worker workerData round-trip', async () => {
    ...
    const workerPath = path.join(__dirname, 'claude-contract-worker-ping.mjs');
  ...
  await testAsync('worker_threads bridge: isMainThread + environment data + MessageChannel', async () => {
    ...
    wt.setEnvironmentData('claude-contract', { k: 1 });
    assertDeepEq(wt.getEnvironmentData('claude-contract'), { k: 1 });
