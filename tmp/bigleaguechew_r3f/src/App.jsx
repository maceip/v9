...
const project = {"name":"bigleaguechew","title":"Gradient 2","type":"bigleaguechew","screenshot":"r3f.maximeheckel.com/Screenshot 2026-03-06 234753.png","copy":[]};
...
const ShaderPlaneMaterial = shaderMaterial(
  ...
  fullScreenVertex,
  /* glsl */ "\nprecision highp float;\nvarying vec2 vUv;\nuniform float uTime;\nvoid main(){ vec2 uv = vUv; vec2 p = uv - 0.5; float r1 = length(p - vec2(0.18*sin(uTime*0.2), 0.16*cos(uTime*0.25))); float r2 = length(p + vec2(0.25*cos(uTime*0.22), 0.1*sin(uTime*0.18))); vec3 blue = vec3(0.08,0.39,0.98); vec3 pink = vec3(0.85,0.42,0.73); vec3 col = mix(blue, pink, 0.5 + 0.5*sin((uv.x+uv.y)*2.2 + uTime*0.15)); col = mix(col, blue, smoothstep(0.08, 0.7, r1)); col = mix(col, pink, smoothstep(0.08, 0.65, r2)); gl_FragColor = vec4(col,1.0); }"
);
...
function TitleOverlay() {
  return (
    <div className="overlay">
      <div className="title">Gradient 2</div>
...
export default function App() {
  const cam = cameraProps("bigleaguechew");
  const isShader = ['artreyu','berghain','bigleaguechew','gaudi','howhigh','laserprinter','trumanshow','tumble','flight','billandted'].includes("bigleaguechew");
  const intensive = ['spacex', 'berghain', 'howhigh', 'billandted'].includes("bigleaguechew");
  return (
    <>
      <TitleOverlay />
      <Canvas dpr={intensive ? 1 : [1, 1.75]} gl={{ antialias: true, powerPreference: "high-performance", preserveDrawingBuffer: true }}>
        ...
        {!isShader && ['simcity','wipeout'].includes("bigleaguechew") && <OrbitControls enablePan={false} enableZoom={false} autoRotate autoRotateSpeed={0.4} />}
        {!isShader && "bigleaguechew" === 'narnia' && <OrbitControls enablePan={false} autoRotate autoRotateSpeed={0.3} />}
        <Scene project={project} />
        {!isShader && <Environment preset="city" />}
      </Canvas>
      {"bigleaguechew" === 'fasttimes' && <div className="crt-overlay" />}
    </>
  );
}
