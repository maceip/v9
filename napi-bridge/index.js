...
`);
  ...
  return {
     ...
     */
    eval(code) {
      const result = executeScript(EVAL_HELPER_PATH, {
        __edge_code: String(code),
      });

      if (result.error) {
        const err = new Error(`eval failed: ${result.error}`);
     ...
     */
    runFile(path) {
      const result = executeScript(RUNFILE_HELPER_PATH, {
        __edge_target: String(path),
      });
